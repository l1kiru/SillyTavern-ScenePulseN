...
const { getSnapshotFor, forceFullStateRefresh, shouldUseDelta, clearForceFullState } = await import('../src/settings.js');
...
ctx.extensionSettings.scenepulse.deltaMode=true;
forceFullStateRefresh();
const cancelledParallel=generateTracker(9);
const stopCallsBeforeParallelCancel=stopGenerationCalls;
setTimeout(()=>{
    cancelGeneration();
    eq('forced full is rearmed synchronously on Stop',shouldUseDelta(parallelTracker),false);
},20);
const cancelledParallelResult=await cancelledParallel;
eq('forced full remains pending after cancelled request settles',shouldUseDelta(parallelTracker),false);
clearForceFullState();
ctx.extensionSettings.scenepulse.deltaMode=false;
