...
eq('full output has safe minimum',computeResponseLength({mode:'full'}),16384);
eq('delta output has safe minimum',computeResponseLength({mode:'delta'}),8192);
eq('large snapshot budget is capped',computeResponseLength({mode:'full',previousSnapshot:{text:'x'.repeat(40000)}}),16384);
eq('truncation increases budget',computeResponseLength({mode:'delta',attempt:1,lastErrorCode:'TRUNCATED'})>8192,true);
