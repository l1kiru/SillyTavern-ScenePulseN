...
export async function processExtraction(mesIdx, extracted, source, opts = {}) {
    ...
    const responseCharacterNames = (extracted.characters || []).map(c => c.name);
    ...
    const rosterKeys = new Set((extracted.charactersPresent ?? responseCharacterNames).map(characterNameKey));
    const currentScene = { ...extracted, characters: (extracted.characters || []).filter(c =>
        [c.name, ...(c.aliases || [])].some(name => rosterKeys.has(characterNameKey(name))),
    ) };
    const _audienceValidation=validateCharacterAudienceRequirements(currentScene,{
    ...
    saveSnapshot(mesIdx, norm, targetSwipeId,{customFieldSpecs:opts.frozenCharacterCustomFieldSpecs});
