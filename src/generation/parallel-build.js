...
// Presence is operational state even when its dashboard row is hidden.
export function withOperationalRoster(fullSchema) {
    const result = clone(fullSchema);
    const root = schemaRoot(result);
    if (root.properties.characters && !root.properties.charactersPresent) {
        root.properties.charactersPresent = {
            type: 'array', items: { type: 'string' },
            description: 'NPCs physically present with {{user}} now. Exclude the user, memories and off-scene characters. Return [] for solitude.',
        };
        root.required = [...(root.required || []), 'charactersPresent'];
    }
    return result;
}
    ...
    for (const gap of gaps) {
        if (gap.kind === 'characters') {
            for (const name of gap.names || []) {
                ...
                mergedValue.characters.push(carried);
                present.add(key);
                filled.push(name);
            }
            ...
            for (const field of gap.fields || []) {
                ...
                if (previousSnapshot && Object.hasOwn(previousSnapshot, field)) {
                    ...
                    if (field === 'relationships') for (const rel of mergedValue[field] || []) {
                        for (const key of ['lastReaction', 'changeReason']) if (Object.hasOwn(rel, key)) rel[key] = '';
                    }
                    filled.push(field);
                }
            }
...
function lanePrompt({ spec, schema, fullSchema, contextText, previousSnapshot, coreResult, retryInstruction = '' }) {
    ...
    const routerRule = spec.kind === 'core' && spec.fields.includes('sceneTags')
        ? `\nsceneTags and resolvedTags are arrays using only these canonical tags: ${SCENE_TAG_REGISTRY.join(', ')}. Multiple tags may coexist. Return [] when none apply. Activate current situations immediately. Put injury in resolvedTags only when the narrative explicitly establishes that the injury is resolved.`
        : '';
...
async function executeLane({
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        let rawText = '';
        try {
            const routed = applyPromptRole({ systemPrompt: laneSystemPrompt(systemPrompt, spec), prompt }, promptRole);
            const response = await request({
                profileId,
                messages: [
                    ...(routed.systemPrompt ? [{ role: 'system', content: routed.systemPrompt }] : []),
...
function freezeLaneRequest(request, stContext, sources) {
    const expand = freezePromptMacros(stContext ?? globalThis.SillyTavern?.getContext?.(), sources);
    return args => request({
        ...args,
        messages: args.messages.map(message => ({ ...message, content: expand(message.content) })),
        jsonSchema: expandSchemaDescriptions(args.jsonSchema, expand),
    });
}
...
export async function runParallelFullBuild({
    ...
    if (signal?.aborted) throw signal.reason || new DOMException('Parent build aborted', 'AbortError');
    fullSchema = withOperationalRoster(fullSchema);
    request = freezeLaneRequest(request, stContext, [systemPrompt, contextText, fullSchema, previousSnapshot, panels, macroSourceText]);
    const started = now();
    ...
    const core = await executeLane({
        ...
        signal, request, service, stContext, timing, characterCustomFieldSpecs, promptRole,
    });
...
export async function runParallelDeltaBuild({
    ...
    const validationSchema = activation
        ? buildPanelStateValidationSchema(fullSchema, panels, activation.inactivePanelIds)
        : fullSchema;
    // Archived off-scene characters had no lane this turn. Validate the
    // current roster; old archive entries are not new-panel initialization.
    const rosterKeys = new Set(plan.roster.map(characterNameKey));
    const currentScene = { ...merged, characters: (merged.characters || []).filter(character =>
        [character.name, ...(character.aliases || [])].some(name => rosterKeys.has(characterNameKey(name))),
    ) };
    const finalValidation = validateExtraction(currentScene, { schema: schemaRoot(validationSchema) });
    const audienceValidation = validateCharacterAudienceRequirements(currentScene, {
        schema: schemaRoot(validationSchema), customFieldSpecs: characterCustomFieldSpecs,
        mode: 'full', activePanelIds,
    });
    finalValidation.errors.push(...audienceValidation.errors);
