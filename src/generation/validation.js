...
export function validateCharacterAudienceRequirements(data, {
    ...
    if((mode==='delta'&&previousSnapshot===undefined)||!data||!Array.isArray(data.characters))return{valid:true,errors,warnings};
    let active=schema;
    ...
    for(let i=0;i<data.characters.length;i++){
        ...
        for(const spec of specs){
            const keys=new Set([character.name,...(character.aliases||[])].map(characterNameKey));
            const prior=(previousSnapshot?.characters||[]).find(item=>[item.name,...(item.aliases||[])].some(name=>keys.has(characterNameKey(name))));
            const matches=characterMatchesAudience({...prior,...character},spec.audience);
            const present=Object.hasOwn(character,spec.key)&&character[spec.key]!==null&&character[spec.key]!==undefined;
            const mayCarry=mode==='delta'&&!newlyActivePanelIds.includes(spec.panelId)&&prior&&Object.hasOwn(prior,spec.key);
            if(matches&&!present&&!mayCarry){
