...
import {
    generating, genNonce, genMeta, lastGenSource, inlineGenerationContext,
...
export function cancelGeneration(){
    ...
    if(inlineGenerationContext?.fullRefreshTicket){
        rearmForceFullAfterFailedFullRun(inlineGenerationContext.frozenDeltaMode,inlineGenerationContext.fullRefreshTicket);
    }
...
export async function generateTracker(mesIdx,partKey,opts){
    ...
    const fullRefreshTicket=consumeForceFull?clearForceFullState():null;
    const rearmOnFail=()=>{if(consumeForceFull)rearmForceFullAfterFailedFullRun(useDelta,fullRefreshTicket)};
    ...
    const frozenPromptRole=getActivePromptRole();
    const macroSourceText=assemblePrompt(frozenPromptView,frozenProfile,promptOptions);
    ...
    requestAbort.signal.addEventListener('abort',rearmOnFail,{once:true});
    ...
    if(result){
        ...
        saveSnapshot(mesIdx,result,targetSwipeId,{customFieldSpecs:characterCustomFieldSpecs});
