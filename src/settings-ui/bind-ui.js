...
function _syncParallelControls(s=getSettings()){
    ...
    const presetInfo=document.getElementById('sp-parallel-preset-info');
    if(presetInfo)presetInfo.style.display=enabled?'block':'none';
...
export function bindUI(){const s=getSettings();
    ...
    $('#sp-injection-method').on('change',function(){const previous=captureTrackerStructure();s.injectionMethod=this.value;reconcileTrackerStructureChange(previous);saveSettings();_spSaveLS();$('#sp-method-inline').toggle(this.value==='inline');$('#sp-method-separate').toggle(this.value!=='inline');$('#sp-embed-section').toggle(this.value!=='inline');$('#sp-separate-settings').toggle(this.value!=='inline');refreshOpenPanelManager()});
    ...
    $('#sp-scene-source-trace').on('change',function(){
        s.sceneSourceTrace=this.checked;
        saveSettings();
        const snap=getLatestSnapshot();
    ...
    $('#sp-btn-gen').on('click',async()=>{const ctx=SillyTavern.getContext();const mesIdx=getLastAssistantMessageIndex(ctx);if(!canGenerateScene(ctx,mesIdx)){toastr.info(t('Open a chat before generating ScenePulse.'));return}toastr.info(t('Generating…'));
