...
import {
    ...
    getRequestedPanelActivationStrategy,
} from '../settings.js';
import {
    customPanelScope,
...
function localizedGender(gender) {
    return { female: t('Female'), male: t('Male'), nonbinary: t('Nonbinary') }[gender] || gender;
}

function sceneTagLabel(tag) {
    return {
        social: t('Social'), combat: t('Combat'), injury: t('Injury'), nsfw: t('NSFW'),
        stealth: t('Stealth'), travel: t('Travel'), investigation: t('Investigation'),
        magic: t('Magic'), vehicle: t('Vehicle'), medical: t('Medical'), rest: t('Rest'),
    }[tag] || tag;
}

function audienceLabel(audience) {
    return [
        audience.names.length ? t('Names') + ': ' + audience.names.join(', ') : '',
        audience.genders.length ? t('Gender') + ': ' + audience.genders.map(localizedGender).join('/') : '',
        audience.keywords.length ? t('Keywords') + ': ' + audience.keywords.join(', ') : '',
    ].filter(Boolean).join('; ');
}
...
export function renderCustomPanelsMgr(s,container,panelBody){
    ...
    const panelControl=getRequestedPanelActivationStrategy(s);
    ...
    if(panelControl==='automatic'&&(s.parallelFullGeneration!==true||s.injectionMethod!=='separate')){
        _appendWarning(container,'sp-cp-warn','#f59e0b',t('Automatic panel selection requires Separate mode with parallel tracker requests enabled.')+' '+t('Enabled panels use manual selection until automatic routing is available.'));
    }
    if(!panels.length){
    ...
    panels.forEach((cp,cpIdx)=>{
        ...
        if(customPanelScope(cp)==='character'){
            ...
            const hintText=audienceLabel(audience);
            ...
            for(const gender of CHARACTER_GENDER_OPTIONS){
                ...
                const text=document.createElement('span');text.textContent=localizedGender(gender);
        ...
        if(panelControl==='automatic'&&activationMode==='auto'){
            ...
            tagsSummary.textContent=selectedTags.size?[...selectedTags].map(sceneTagLabel).join(' · '):t('Scene tags');
            ...
            for(const tag of SCENE_TAG_REGISTRY){
                ...
                const text=document.createElement('span');text.textContent=sceneTagLabel(tag);
                checkbox.addEventListener('change',()=>{
                    if(!_commitPanelEdit(panels,next=>{
