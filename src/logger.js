...
/** Read live setting without importing settings.js (avoids circular deps). */
export function isConsoleDebug() {
    try {
        return !!SillyTavern.getContext()?.extensionSettings?.[MODULE_NAME]?.consoleDebug;
    } catch {
        return false;
    }
}
...
export function log(...a){if(isConsoleDebug())console.log(LOG,...a);_push('',a)}
export function warn(...a){if(isConsoleDebug())console.warn(LOG,...a);_push('WARN',a)}
export function err(...a){
    if(isConsoleDebug())console.error(LOG,...a);_push('ERROR',a);
