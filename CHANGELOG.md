...
- Parallel requests resolve prompt/schema macros before transport, preserve the selected
  prompt role, and embed descriptive snapshots when enabled. Core still identifies NPCs
  when the visible presence list is disabled; routing instructions include `rest`.
- Newly activated audience panels require initial values only for current-scene matching
  characters. Frozen schemas and custom-field contracts survive in-flight settings edits.
- Full refresh survives cancellation, failure and Together chat changes without changing
  another operation's state. Partial updates show a RU/EN notice and clear carried
  transient thoughts, intentions and reactions.
- Unavailable automatic routing falls back to manual selection. Settings explain which
  Connection Manager preset parallel requests use. Parallel defaults are unchanged;
  live-provider performance has not been measured for this integration.
